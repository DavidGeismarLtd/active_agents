...
module PromptTracker
  RSpec.describe SyncOpenaiAssistantsToAgentVersionsService, type: :service do
    ...
    describe "#call" do
      ...
      it "creates AgentVersions from OpenAI assistants" do
        expect {
          described_class.new.call
        }.to change(AgentVersion, :count).by(2)
      end
      ...
      context "when Prompt creation fails" do
        before do
          ...
          allow_any_instance_of(Agent).to receive(:save!).and_raise(ActiveRecord::RecordInvalid.new)
        end
      ...
      context "when API key is not configured" do
        ...
        it "raises SyncError" do
          expect {
            described_class.new.call
          }.to raise_error(SyncOpenaiAssistantsToAgentVersionsService::SyncError, /OpenAI API key not configured/)
        end
      end
