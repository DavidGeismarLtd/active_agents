...
module PromptTracker
  module Monitoring
    ...
    class AgentVersionsController < BaseController
      # GET /monitoring/prompts/:agent_id/versions/:id
      # Show prompt version with all tracked calls and evaluations
      def show
        @prompt = Agent.find(params[:agent_id])
        @version = @prompt.agent_versions.find(params[:id])
        ...
        all_evaluations = Evaluation.tracked.joins(:llm_response)
                                    .where(prompt_tracker_llm_responses: { agent_version_id: @version.id })
