...
module PromptTracker
  ...
  #
  # @example Usage in TaskAgentRuntimeService
  #   # Initialize with server names from PromptVersion
  #   manager = McpClientManager.new(["filesystem", "weather_api"])
  #   manager.connect_all
  #
  #   # Get all tools for LLM payload
  ...
  class McpClientManager
    ...
    def connect_all
      ...
      server_names.each do |server_name|
        begin
          ...
          results[server_name] = { success: false, error: e.message }
          Rails.logger.error "[McpClientManager] Failed to connect to #{server_name}: #{e.message}"
          raise ManagerError, "Failed to connect to MCP server '#{server_name}': #{e.message}"
        end
      end
    ...
    # @raise [ManagerError] if tool discovery fails
    def list_all_tools
      all_tools = []
      ...
      @connections.each do |server_name, connection|
        begin
          ...
          # Convert each tool to LLM format with prefix
          ...
          Rails.logger.error "[McpClientManager] Failed to list tools from #{server_name}: #{e.message}"
          raise ManagerError, "Failed to list tools from '#{server_name}': #{e.message}"
        end
      end
    ...
    # @raise [ManagerError] if execution fails
    def call_tool(tool_name, arguments)
      ...
      Rails.logger.info "[McpClientManager] Calling #{server_name}::#{original_tool_name} with #{arguments.inspect}"
    ...
    #
    # @param server_name [String] server name
    # @param tool [Hash] MCP tool definition
    # @return [Hash] tool in OpenAI/Anthropic format
