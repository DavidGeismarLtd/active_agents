...
module PromptTracker
  ...
  class AgentRuntimeService
    ...
    def initialize_mcp_manager
      server_names = deployed_agent.prompt_version.mcp_server_names
    ...
    def mcp_tool?(function_name)
      ...
      # MCP tools are prefixed with server name: "server__tool"
    ...
    def execute_mcp_tool(function_name, arguments)
      ...
      Rails.logger.info "[AgentRuntimeService] 🔌 Arguments: #{arguments.inspect}"

      start_time = Time.current

      # Call the MCP manager
      ...
      # Create FunctionExecution record for timeline tracking
      # MCP tools don't have a FunctionDefinition, so function_definition_id is nil
      FunctionExecution.create!(
        deployed_agent: deployed_agent,
        agent_conversation: @conversation,
        function_name: function_name,
        function_definition_id: nil,  # MCP tools are virtual
        arguments: arguments,
        result: result,
        success: success,
      ...
      # Return result in format expected by RubyLLM
      result
    rescue StandardError => e
