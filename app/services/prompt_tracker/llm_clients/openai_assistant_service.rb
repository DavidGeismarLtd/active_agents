...
module PromptTracker
  module LlmClients
    ...
    #   response = LlmClients::OpenaiAssistantService.call(
    ...
    class OpenaiAssistantService
      ...
      def call
        ...
        Rails.logger.info "[LlmClients::OpenaiAssistantService] Using thread_id: #{thread_id} (existing: #{existing_thread_id.present?})"
      ...
      def retrieve_response(thread_id, run_id)
        ...
        Rails.logger.info "[LlmClients::OpenaiAssistantService] Messages response: #{messages_response.inspect}"

        assistant_message = messages_response["data"].first
        Rails.logger.info "[LlmClients::OpenaiAssistantService] Assistant message: #{assistant_message.inspect}"

        # Extract text content - may have annotations from file_search
        content_block = assistant_message.dig("content", 0)
        Rails.logger.info "[LlmClients::OpenaiAssistantService] Content block: #{content_block.inspect}"

        content = content_block&.dig("text", "value")
        annotations = content_block&.dig("text", "annotations") || []
        Rails.logger.info "[LlmClients::OpenaiAssistantService] Annotations: #{annotations.inspect}"

        # Get usage from the run
        run = client.runs.retrieve(thread_id: thread_id, id: run_id)
        Rails.logger.info "[LlmClients::OpenaiAssistantService] Run: #{run.inspect}"

        usage = run["usage"] || {}

        # Extract results from run steps
        run_steps = client.run_steps.list(thread_id: thread_id, run_id: run_id, parameters: { order: "asc" })
        Rails.logger.info "[LlmClients::OpenaiAssistantService] Run steps: #{run_steps.inspect}"
